...
// #if defined(USE_FP16)
    if(use_type=="USE_FP16"){
        config->setFlag(BuilderFlag::kFP16);
    }
// #elif defined(USE_INT8)
    else if(use_type=="USE_INT8"){
        std::cout << "Your platform support int8: " << builder->platformHasFastInt8() << std::endl;
        assert(builder->platformHasFastInt8());
        config->setFlag(BuilderFlag::kINT8);
        Int8EntropyCalibrator2 *calibrator = new Int8EntropyCalibrator2(1, INPUT_W, INPUT_H, "./calib/", "mnet_int8calib.table", INPUT_BLOB_NAME);
...
int main(int argc, char** argv) {
    if (argc < 2) {
        ...
        std::cerr << "./retina_mnet -s -USE_INT8  [-USE_INT8 or USE_FP16 or USE_FP32]// serialize model to plan file" << std::endl;
    ...
    if (std::string(argv[1]) == "-s") {
        if(argc !=3 ){
            std::cerr << "arguments not right!" << std::endl;
            std::cerr << "./retina_mb025 -s -USE_INT8   // serialize model to plan file" << std::endl;
            return -1;
        }
        std::string sufix = "_fp32";
        if(std::string(argv[2]) == "-USE_INT8"){
            use_type = "USE_INT8";
            sufix = "_int8";
        }
        else if(std::string(argv[2]) == "-USE_FP16"){
            use_type = "USE_FP16";
            sufix = "_fp16";
        }
        else {
        ...
        std::string engine_name = "../engine/retina_mb025_" + std::to_string(BATCH_SIZE) 
                                    + "_" + std::to_string(INPUT_W)+"x" +std::to_string(INPUT_H)
                                    + sufix + ".engine";
