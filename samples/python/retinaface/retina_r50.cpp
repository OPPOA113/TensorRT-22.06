...
// #elif defined(USE_INT8)
        ...
        Int8EntropyCalibrator2 *calibrator = new Int8EntropyCalibrator2(1, INPUT_W, INPUT_H, "./calib/", "r50_int8calib.table", INPUT_BLOB_NAME);
...
int main(int argc, char** argv) {
    if (argc < 2) {
        ...
        std::cerr << "./retina_r50 -s -USE_INT8   // serialize model to plan file" << std::endl;
    ...
    if (std::string(argv[1]) == "-s") {
        if(argc !=3 ){
            std::cerr << "arguments not right!" << std::endl;
            std::cerr << "./retina_r50 -s -USE_INT8 [-USE_INT8 or USE_FP16 or USE_FP32]  // serialize model to plan file" << std::endl;
            return -1;
        }
        ...
        std::string engine_name = "../engine/retina_r50_" + std::to_string(BATCH_SIZE) 
