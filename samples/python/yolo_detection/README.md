# yolo v3 v5系列和effecientDet in TensorRT
 - yolo系列模型在tensorrt上的使用：pth->onnx->engine->infer->map
 - onnx生成：在原仓库的相关脚本生成
 - engine生成：build_engine.py 支持fp16 int8 batchsize input_size
 - eval_yolo.py 测试trt模型，推理或者计算map

 - infer 生成测试图片结果 以txt生成
 - map计算

```
python3 build_engine.py \
	--batch_size 1 \
    --onnx /workspace/project/projects/yolov5-6.0/yolov5_onnx/yolov5s/yolov5s_1_320.onnx \
    --engine /workspace/project/projects/yolov5-6.0/yolov5_engine/yolov5s_1_320_fp16.engine \
    --precision fp16

python3 build_engine.py \
	--batch_size 1 \
    --onnx /workspace/project/projects/yolov5-6.0/yolov5_onnx/yolov5s/yolov5s_1_320.onnx \
    --engine /workspace/project/projects/yolov5-6.0/yolov5_onnx/yolov5s/yolov5s_1_320_int8.engine \
    --precision int8 \
	--calib_input /workspace/project/dataset/benchmarch_data/detection/coco_calib \
    --calib_cache /workspace/project/projects/yolov5-6.0//yolov5_onnx/yolov5s/yolov5s.cache
------------infer-------------------
python eval_yolo.py \
	--engine /workspace/project/projects/yolov5-6.0/yolov5_engine/yolov5s/yolov5s_1_320_int8.engine \
    --inputsize 320 \
    --input /workspace/project/dataset/benchmarch_data/detection/test/ \
    --include infer \
    -o ./output
...
python eval_yolo.py \
	--engine /workspace/project/projects/yolov5-6.0/yolov5_engine/yolov5s/yolov5s_1_320_int8.engine \
    --inputsize 320 \
    --input /workspace/project/dataset/benchmarch_data/detection/val2017 \
    --include map \
...
trtexec \
    --loadEngine=/workspace/project/projects/yolov5-6.0/yolov5_onnx/yolov5s/yolov5s_8_320.engine \
    --useCudaGraph --noDataTransfers \
    --duration=30 \
    --batch=8   

python3 infer.py --engine /workspace/project/projects/yolov5-6.0/yolov5_onnx/yolov5s/yolov5s_8_320.engine \
    --input /workspace/project/dataset/benchmarch_data/detection/test \
    --output ./output \
    --nms_threshold 0.25
