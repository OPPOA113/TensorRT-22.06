...
通用的分类网络模型，在`tensorrt-8.2.5`上的`benchmark`测试`pipeline`：

1. onnx模型生成，从原算法模型仓库中生成；

2. 本脚本下，将onnx模型转换到trt序列化模型：fp32 fp16 int8精度

3. trt序列化的模型inference, 并计算模型的map
...
```
STEP1: pth-->onnx
python cls_mode_hub.py --batchsize 1 --model_library torchvision --pretrained_weights ./resnet50/resnet50-19c8e357.pth --save_dir ./resnet50/ --model_name resnet50 
-----------------------------------
STEP2: onnx-->engine
python build_engine.py \
    --onnx /path/to/model_size.onnx \
    --engine /path/to/model_size_int8.trt \
    --precision int8 \
...
e.g. --int8
python build_engine.py --onnx ./resnet50/resnet50_1_224.onnx --engine ./resnet50/resnet50_1_224_int8.trt --precision int8 --calib_input /workspace/project/dataset/benchmarch_data/recognise/ILSVRC2012_img_calib --calib_cache ./resnet50/resnet50.cache --calib_preprocessor RESNET
e.g. --fp16
python build_engine.py --onnx ./resnet50/resnet50_1_224.onnx --engine ./resnet50/resnet50_1_224_fp16.trt --precision fp16
-----------------------------------
STEP3: trt infer
